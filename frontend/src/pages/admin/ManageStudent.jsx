...
export default function StudentManagement() {
      ...
      try {
        ...
        const normalized = response.data.map((f) => {
          ...
          return {
            ...
            // ✅ FIX email fallback
            universityEmail: f.universityEmail ?? f.university_email ?? "",

            personalEmail: f.personalEmail ?? f.personal_email ?? "",
  ...
  const filtered = studentData.filter((s) => {
    ...
    const matchesProgram =
      programFilter === "All Programs" || s.program === programFilter;
  ...
  const semesters = [...new Set(studentData.map((s) => s.semester))].sort(
    (a, b) => a - b,
  );
      ...
      <div className={`sm-page${sidebarOpen ? " shifted" : ""}`}>
        ...
        <div className="sm-filters">
          ...
          >
            ...
            {programs.map((prog) => (
              <option key={prog._id} value={prog.programName}>
                {prog.programName}
              </option>
            ))}
          </select>
                ...
                {!loading &&
                  ...
                  filtered.map((s, i) => (
                    <tr
                      key={s.id}
                      className={
                        selectedStudent?.id === s.id ? "sm-row-selected" : ""
                      }
                      onClick={() => openProfile(s)}
                      style={{ cursor: "pointer" }}
                    >
                      <td>
                        <span className="sm-id-text">{s.id}</span>
                      </td>
                      <td>
                      ...
                      <td>{s.program || "—"}</td>
                      <td>
                        <span className="sm-sem-pill">{s.semester || "—"}</span>
                      </td>
                      <td>{s.division || "—"}</td>
                      <td>{s.batch || "—"}</td>
                      <td>
                      ...
                      <td>
                        <div
                          className="sm-action-btns"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <button
                            className="sm-act-btn sm-act-view"
                            onClick={() => openProfile(s)}
                          >
                            <Eye size={13} />
                          </button>
